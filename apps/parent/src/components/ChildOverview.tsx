...
const chromePills = [
  { label: "Прогресс", icon: ChartBarIcon },
  { label: "Предметы", icon: BookOpenIcon },
  { label: "Квесты", icon: AcademicCapIcon },
  { label: "AI советы", icon: LightBulbIcon },
  { label: "Ритм", icon: CalendarDaysIcon },
];

const monthLabels = ["Авг", "Сен"];
    ...
    <section className="overflow-hidden rounded-[34px] border border-[#d8d8d2] bg-[#d8d8d0] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.35)]">
      <div className="rounded-t-[34px] bg-[#3f403d] px-4 pb-0 pt-4">
        <div className="flex items-center gap-2">
          <span className="grid size-8 place-items-center rounded-full bg-[#f2f2ec] text-sm text-[#555651]">
            x
          </span>
        ...
        <div className="relative z-10 grid gap-6 lg:grid-cols-[170px_1fr]">
          ...
          <div className="min-w-0">
            <div className="mb-5 grid gap-4 lg:grid-cols-[1fr_auto]">
              <div>
                <p className="text-xs font-medium uppercase text-[#77786f]">Диагностика</p>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4 lg:min-w-[520px]">
                ...
                <Metric label="Время" value={`${studyHours}ч ${studyMinutes}м`} />
                <Metric label="XP" value={child.xp.toString()} />
                <Metric label="Серия" value={`${child.streakDays}д`} />
              </div>
            </div>

            <div className="mb-6 flex flex-wrap items-center gap-2">
              {["Все", "Предметы", "Квесты", "Рекомендации", "Активность"].map((item, index) => (
                <span
                  key={item}
        ...
        <div className="relative z-10 mt-6 grid gap-5 lg:grid-cols-2">
          {monthLabels.map((month, index) => (
            <TimelineColumn
              key={month}
              month={month}
              week={index === 0 ? "I неделя" : "II неделя"}
...
function ProfileCard({
  ...
  return (
    <div className="rounded-[26px] bg-white px-4 py-4 shadow-[0_14px_30px_rgba(35,36,32,0.05)]">
      ...
      <p className="mt-3 text-[11px] font-medium text-[#77786f]">Ученик</p>
  ...
  return (
    <div className="relative min-h-[360px] rounded-[28px] border border-[#d4d4ca] bg-[#ecece2]/75 p-5">
      ...
      <div className="mb-6 pl-8">
        <p className="text-sm font-semibold text-[#20211f]">{month}</p>
  ...
  return (
    <div className="rounded-[28px] bg-white p-5 shadow-[0_18px_35px_rgba(37,38,34,0.05)]">
      <div className="mb-5 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-[#1c1d1a]">Активность</h2>
  ...
  return (
    <div className="rounded-[28px] bg-white p-5 shadow-[0_18px_35px_rgba(37,38,34,0.05)]">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-[#1c1d1a]">Сильные стороны</h2>
      </div>
